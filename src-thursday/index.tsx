...
//
//                     <div><img src-thursday={photo?.url} alt=""/></div>
//
//                 </div>
//
//                 <button style={{marginLeft: '15px'}}
...
//                         <b>title</b>: {p.title}
//
//                         <div><img src-thursday={p.url} alt=""/></div>
//
//                     </div>
//
//                 })
...
//
//                 {/!*{getTrue() && <div style={{color: 'red'}}>ERROR</div>}*!/}
//                 {formik.errors.email && <div style={{color: 'red'}}>ERROR</div>}
//
//             </div>
//
//             <button type="submit">Отправить</button>
...
// Попробуйте обновить пост и вы увидите alert с ошибкой. Ответ: const currentPost = getState().posts.find((p: PostType) => p.id === postId)
...
// Для обработки успешного результата написана утилитная функция baseSuccessHandler. Ответ: baseSuccessHandler(dispatch,getTodosAC,res.data) baseSuccessHandler(dispatch,getUsersAC,res.data.items)
...
// Задача: вывести сообщение об ошибке, которую возвращает сервера говорящую о том что email или password некорректны. Ответ: dispatch(setError(e.response.data.errors))
...
// вас всегда редиректило на страницу профиля и при в это в урле по итогу Ответ:  <Route path={'*'} element={<Navigate to ={'profile'}/>}/>
...
// Ваша задача стоит в том чтобы правильно передать нужные типы в дженериковый тип ThunkAction<any, any, any, any>. Ответ: void RootState unknown ActionsType
...
// Задача: при успешной логинизации, редиректнуть пользователя на страницу Profile. Ответ: {isLoggedIn && navigate('profile')}
...
// При загрузке приложения вы должны увидеть список тудулистов, Ответ: dispatch(getTodosTC())
...
// Ваша задача при первом клике задизаблить кнопку удаления, Ответ: disabled={p.isDisabled}
...
// Исправьте 46 строку кода так, чтобы: Ответ:  {formik.touched.email && formik.errors.email && <div style={{color: 'red'}}>{formik.errors.email}</div>}
...
// Напишите запрос на сервер для создания нового комментария. Ответ: return instance.post('comments', payload)
